...
let PdfService = class PdfService {
    ...
    async generateCompanyCostReport(date) {
        ...
        for (const company of companies) {
            ...
            for (const emp of company.employee) {
                ...
                const vt = emp.ticket.reduce((sum, item) => sum + Number(item.value), 0);
    ...
    async generateEmployeeCostReport(date) {
        ...
        doc
            .fontSize(12)
            .text(`Mês de referência: ${dateSelected.toLocaleDateString('pt-BR', {
            month: 'long',
            year: 'numeric',
        })}`, { align: 'center' })
            .moveDown(2);
        ...
        doc
            ...
            .moveDown(0.5)
            .text(`TOTAL GERAL DO MÊS: ${formatCurrency(totalGeral)}`, startX, y + 30, {
            align: 'right',
        });
        doc.end();
        return new Promise((resolve) => {
    ...
    async generateEmployeeCostReportByPeriod(startDate, endDate) {
        const start = new Date(startDate);
        start.setDate(start.getDate() + 1);
        const end = new Date(endDate);
        end.setDate(end.getDate() + 1);
        ...
        doc.on('data', buffers.push.bind(buffers));
        const formatCurrency = (value) => value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
        const diasNoPeriodo = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) + 1;
        const diasUteis = Array.from({ length: diasNoPeriodo }, (_, i) => {
            const d = new Date(start);
            d.setDate(start.getDate() + i);
            return !(0, date_fns_1.isWeekend)(d);
        ...
        doc
            ...
            .moveDown(0.5)
            .text(`TOTAL GERAL DO PERÍODO: ${formatCurrency(totalGeral)}`, startX, y + 30, {
            align: 'right',
        });
        doc.end();
        return new Promise((resolve) => {
    ...
    async generateEmployeeCostReportWithAbsencesByPeriod(startDate, endDate) {
        const start = new Date(startDate);
        start.setDate(start.getDate() + 1);
        const end = new Date(endDate);
        end.setDate(end.getDate() + 1);
